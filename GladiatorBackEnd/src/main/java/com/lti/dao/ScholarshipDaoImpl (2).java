...
public class ScholarshipDaoImpl implements ScholarshipDao {
	...
	public StudentRegistrationDetails readStudentDetailsByEmail(String email) {
		String jpql="select s from StudentRegistrationDetails s where s.login.studentEmailId= :email";
		Query query = entityManager.createQuery(jpql).setParameter("email",email);
		StudentRegistrationDetails s =(StudentRegistrationDetails)query.getSingleResult();
		return s;
	}
	...
	public Institute readInstituteByDiseCode(String diseCode) {
		String jpql = "select i from Institute i where i.login.diseCode=:diseCode";
		Query query = entityManager.createQuery(jpql).setParameter("diseCode", diseCode);
