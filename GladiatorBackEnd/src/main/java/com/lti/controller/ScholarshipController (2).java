...
public class ScholarshipController {
	...
	public Status addStudent(@RequestBody StudentRegistrationDetails studentRegistration) {
		try {
			...
			if (mail.mailValidate(studentRegistration.getLogin().getStudentEmailId())) {
				mail.send(studentRegistration.getLogin().getStudentEmailId(), "REGISTRATION SUCCESSFUL",
	...
	public Status addStudentApplication(@PathVariable String email, @RequestBody StudentDetails studentApplication) {
		try {
			...
			mail.send(email, "SCHOLARSHIP APPLICATION SUCCESSFUL", "<b> Congratulations!! </b>You have successfully applied for "  + studentApplication.getScholarshipType() + "<br><p>Your Application Tracking Id is: "+studentApplication.getStudentApplicationNo()+"</p>");
			Status status = new Status();
			status.setStatus(StatusType.SUCCESS);
